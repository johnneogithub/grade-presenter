    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Year {{ $classroom->year }} &mdash; Block {{ $classroom->block }}</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($students->isEmpty())
                <div class="text-center py-16 text-gray-500">
                    ...
                    <a href="{{ route('students.create', $classroom) }}" class="text-blue-600 underline mt-2 inline-block">Add your first student</a>
                </div>
            @else
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Course / Year / Block</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase">Lab Grade</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase">Lec Grade</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach($students as $student)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-500">{{ $student->course }} / {{ $student->year }} / {{ $student->block }}</td>
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <form action="{{ route('students.destroy', [$classroom, $student]) }}" method="POST"
                                                ...
                                                @csrf @method('DELETE')
                                                <button class="text-xs bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg">
                                                    Remove
                                                </button>
                                            </form>
                                        </div>
