<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Student &mdash; {{ $student->full_name }}
        </h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow p-8">
                <form action="{{ route('students.update', [$classroom, $student]) }}" method="POST" class="space-y-5">
                    @csrf @method('PUT')
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="year" value="{{ old('year', $student->year) }}"
                                   class="mt-1 w-full rounded-lg border-gray-300 shadow-sm" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Block</label>
                            <input type="text" name="block" value="{{ old('block', $student->block) }}"
                                   class="mt-1 w-full rounded-lg border-gray-300 shadow-sm" required>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="laboratory_grade"
                                   value="{{ old('laboratory_grade', $student->laboratory_grade) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="lecture_grade"
                                   value="{{ old('lecture_grade', $student->lecture_grade) }}"
                                   step="0.01" min="0" max="100"
                                   class="mt-1 w-full rounded-lg border-gray-300 shadow-sm">
                        </div>
                    </div>
